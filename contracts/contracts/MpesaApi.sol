...
contract Restv1 is usingKesholabs {
    StableToken ER;

  constructor (Kesholabs prov, StableToken er) public {
      kesholabs = prov;
      ER = er;
  }

  event LogNewKesholabsQuery(string _description);
  ...
  mapping (bytes32 =>Queuevalues)public Queuemap;
  ...
  {
    if(_result == 1 ){
      emit LogErrorInCallback('error occurred during callback execution .. your transaction did not complete');
    }else{
       uint256 amount =  Queuemap[_myid].amount;
      address  _to =  Queuemap[_myid].sender;
      ER.transfer(_to,amount);
  ...
  function deposit(
    ...
    uint256 _amount
     ...
     public
    {
      require(ER.balanceOf(address(this)) > _amount,'insufficien funds');
      bytes32 queryId = Kesholabs_query(_to, _phoneNumber, _amount);
      Queuevalues storage Q = Queuemap[queryId];
      Q.sender = _to;
      Q.amount = _amount;
  }
  ...
  function deposit(
    ...
    {
      require(ER.balanceOf(address(this)) > _amount,'insufficien funds');
      bytes32 queryId = Kesholabs_query(msg.sender, _phoneNumber, _amount);
      Queuevalues storage Q = Queuemap[queryId];
      Q.sender = msg.sender;
      Q.amount = _amount;
  }
